...
DCRBR_out DCRBR_kmeans(adj_matrix *A, int k, DCRBR_param param) {
    ...
    double UTB[k];
    cblas_dgemv(CblasRowMajor, CblasTrans, nnode, k, 1, U, k, d, 1, 0, UTB, 1);
...
#pragma omp for
            ...
            for (int j = 0; j < nnode; ++j){
                ...
                cblas_daxpy(k, -d[rid], U + rid * k, 1, midb1, 1);
                ...
                alpha = -d[rid]; cblas_daxpy(k, alpha, U + rid * k, 1, UTB, 1); 

                cblas_dcopy(k, xmid, 1, U + rid * k, 1);  /* U(idx(j), :) = xmid; */
            }
    ...
    kmeans(nnode, k, U, k, labels, H, param.k_param);
...
DCRBR_out DCRBR_kmeans_p(adj_matrix *A, int k, DCRBR_param param) {
    ...
    nnzero = A->nnz; nnode = A->n; d = A->d;
    ir = A->pntr; jc = A->indx;
    maxIter = param.maxIter; p = param.p;
    if (param.verbose){
...
#pragma omp for nowait
    ...
    free(U); free(iU); free(index); free(H); free(full_U);
    return out;
 ...
 * @brief compute the objective function value. Requires: U row major, ldu=k
 */
